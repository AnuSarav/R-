...
############################
# All VIF are within bounds of 10 (In linear regression: VIF < 5. in logistic regression, its slightly relaxed: VIF < 10)
...
Test_Prob = predict(M7, Test, type = "response") # type = "response" will give probability values as output
head(Test_Prob)
...
# Prediction produces an output which has attributes like True Positives, FP, TN, etc.

ROC_Curve = performance(ROC_pred, "tpr", "fpr")
# performance(): All kinds of predictor evaluations (like tpr, fpr, auc, etc.) are performed using this function.
